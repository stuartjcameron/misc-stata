...
{p 8 17 2}
{cmdab:impartial} {it:{help varname:outcomevar}}[{cmdab:=}{it:value1} [{it:value2}]], {cmdab:over(}{it:{help varname:groupvar}}[{cmdab:=}{it:groupA} [{it:groupB}]]{cmdab:)} [{cmdab:bar}|{cmdab:hbar} [{cmdab:graphoptions(}{it:{help graph bar:graph_bar_options}}{cmdab:)}]
...
{phang}If the options {cmdab:bar} or {cmdab:hbar} are specified, then the command will output the same comparison as a column or bar chart. Anything in {cmdab:graphoptions} is passed to the {help graph bar} command as additional options.
...
To add a bar chart:
	...
	{cmd:. impartial wage, over(race=black white) hbar}

{pstd}
To override the graph defaults, changing the colour of the bars and the y-axis title:

	{cmd:. impartial wage, over(race=black white) hbar graphoptions(bar(1, color(green)) ytitle("Average hourly wage"))} 
